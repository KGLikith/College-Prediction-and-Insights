...
import { ArrowLeft, Download, Share2, RefreshCw, AlertCircle, Building2, Award, Target, TrendingUp } from "lucide-react"
...
const getCourseCode = (courseName: string): string => {
  ...
  if (Object.values(COURSE_NAME_TO_CODE).includes(upperCourse as any)) return upperCourse
...
export default function PredictionsPage() {
  ...
  useEffect(() => {
    const fetchPredictions = async () => {
      try {
        ...
        if (filters.course !== "all") queryParams.set("course", getCourseCode(filters.course))
        if (filters.chances !== "all") queryParams.set("chances", filters.chances)
        if (filters.round !== "all") queryParams.set("round", filters.round)

        router.replace(`?${queryParams.toString()}`, { scroll: false })

        const res = await axios.get(`/api/predictions/${examType}?${queryParams.toString()}`)
  ...
  /* ---------- TOP 3 ---------- */
  const top3 = useMemo(() => {
    return filteredColleges.filter((c) => c.chances === "high").slice(0, 3)
  }, [filteredColleges])
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto p-6">
      ...
      {/* META */}
      <Card className="bg-muted/50 shadow-md border-2">
        <CardContent className="py-4 flex flex-wrap gap-6 text-sm font-medium">
          <div>
            Exam: <span className="font-bold text-lg text-primary">{examType.toUpperCase()}</span>
          </div>
          <div>
            Rank: <span className="font-bold text-lg">{userRank}</span>
          </div>
          <div>
            Category: <span className="font-bold text-lg">{userCategory}</span>
          </div>
          <div>
            Predictions: <span className="font-bold text-lg text-primary">{filteredColleges.length}</span>
          </div>
        </CardContent>
      </Card>
      ...
      {top3.length > 0 && (
        <div>
          <h2 className="text-2xl font-bold mb-4 text-primary">Top Matches</h2>
      ...
      <ResultsFilter
        ...
        onClearFilters={() => setFilters({ search: "", course: "all", chances: "all", round: "all" })}
      />

      <div>
        {loading ? <TableSkeleton /> : <CollegeTable colleges={filteredColleges} title="All Predictions" />}

        {/* <div className="flex justify-center gap-4 mt-6">
          <Button disabled={page === 1} onClick={() => setPage((p) => p - 1)} variant="outline" className="shadow-sm">
            Previous
          </Button>
